...
if [ -f "$MODPATH/GameUnlockerApp.apk" ]; then
  if command -v magisk >/dev/null; then
    ui_print " [*] Magisk detected: Installing companion WebUI app..."
    mkdir -p "$MODPATH/system/app/GameUnlockerUI"
    mv "$MODPATH/GameUnlockerApp.apk" "$MODPATH/system/app/GameUnlockerUI/GameUnlockerUI.apk"
    set_perm_recursive "$MODPATH/system/app/GameUnlockerUI" 0 0 0755 0644
  else
